...
cevoverse_attach_message <- function(to_load) {
  if (length(to_load) == 0) {
    return(NULL)
  }

  header <- cli::rule(
    left = cli::style_bold("Attaching core cevoverse packages"),
    right = paste0("cevoverse ", package_version_h("cevoverse"))
  )
